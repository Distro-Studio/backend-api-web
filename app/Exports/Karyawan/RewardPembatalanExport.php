...
class RewardPembatalanExport implements FromCollection, WithHeadings, WithMapping
{
    use Exportable;
    ...
    {
        $query = RiwayatPembatalanReward::with(['data_karyawans.users', 'cutis', 'presensis', 'riwayat_izins'])
    ...
    public function map($pembatalanReward): array
    {
        static $no = 1;

        $tgl_pembatalan = Carbon::parse($pembatalanReward->tgl_pembatalan)->format('d-m-Y');

        return [
            $no++,
            optional($pembatalanReward->data_karyawans->users)->nama ?? 'N/A',
            $pembatalanReward->data_karyawans->nik ?? 'N/A',
            $pembatalanReward->tipe_pembatalan,
            $tgl_pembatalan,
            $pembatalanReward->keterangan ?? 'N/A',
            Carbon::parse($pembatalanReward->created_at)->format('d-m-Y H:i:s'),
            Carbon::parse($pembatalanReward->updated_at)->format('d-m-Y H:i:s')
        ];
    }
}
