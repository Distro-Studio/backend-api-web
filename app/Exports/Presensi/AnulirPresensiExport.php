...
class AnulirPresensiExport implements FromCollection, WithHeadings, WithMapping
{
    use Exportable;
    ...
    {
        $query = AnulirPresensi::with(['data_karyawans', 'presensis'])
    ...
    public function map($anulir): array
    {
        static $no = 1;
        ...
        return [
            $no++,
            $anulir->users->nama,
            $anulir->data_karyawans->nik,
            $anulir->alasan ?? 'N/A',
            $anulir->keterangan ?? 'N/A',
            Carbon::parse($anulir->created_at)->format('d-m-Y H:i:s'),
            Carbon::parse($anulir->updated_at)->format('d-m-Y H:i:s')
        ];
    }
}
