...
class NotificationSTR extends Command
{
    ...
    private const CATEGORY_ID = 16;
    private const REMINDER_MONTHS = 7;
    private const DOCUMENT_TYPE = 'str';
    ...
    public function handle(): int
    {
        ...
        foreach ($dataKaryawanList as $karyawan) {
            $expireDate = $this->parseExpireDate($karyawan->masa_berlaku_str, $karyawan->user_id);
        ...
        $this->info("Peringatan masa berlaku STR selesai diproses. Dibuat: {$createdCount}, dilewati: {$skippedCount}.");
    ...
    private function getEligibleEmployees(Carbon $today, Carbon $limitDate): Collection
    {
        return DataKaryawan::query()
            ...
            ->whereRaw(
                "STR_TO_DATE(masa_berlaku_str, '%d-%m-%Y') BETWEEN ? AND ?",
                [
                    $today->format('Y-m-d'),
    ...
    private function parseExpireDate(?string $date, int $userId): ?Carbon
    {
        ...
        try {
            ...
            $this->warn("Format masa_berlaku_str tidak valid untuk user_id {$userId}: {$date}");
            return null;
        }
    }
    ...
    {
        return "Peringatan: Masa berlaku STR Anda akan berakhir pada {$expireDate->format('d-m-Y')}.";
    }
}
