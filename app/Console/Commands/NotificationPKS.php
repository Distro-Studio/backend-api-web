...
class NotificationPKS extends Command
{
    protected $signature = 'app:notification-warning-pks';
    protected $description = 'Notification Warning PKS';

    private const CATEGORY_ID = 17;
    private const REMINDER_MONTHS = 3;
    private const DOCUMENT_TYPE = 'pks';
    ...
    public function handle(): int
    {
        ...
        foreach ($dataKaryawanList as $karyawan) {
            $expireDate = $this->parseExpireDate($karyawan->tgl_berakhir_pks, $karyawan->user_id);
        ...
        $this->info("Peringatan masa PKS selesai diproses. Dibuat: {$createdCount}, dilewati: {$skippedCount}.");
    ...
    private function getEligibleEmployees(Carbon $today, Carbon $limitDate): Collection
    {
        return DataKaryawan::query()
            ...
            ->whereNotNull('tgl_berakhir_pks')
            ->whereRaw(
                "STR_TO_DATE(tgl_berakhir_pks, '%d-%m-%Y') BETWEEN ? AND ?",
                [
                    $today->format('Y-m-d'),
    ...
    private function parseExpireDate(?string $date, int $userId): ?Carbon
    {
        ...
        try {
            ...
            $this->warn("Format tgl_berakhir_pks tidak valid untuk user_id {$userId}: {$date}");
            return null;
        }
    }
            ...
            ->where(function ($query) use ($currentNotificationKey) {
                $query->where(function ($subQuery) use ($currentNotificationKey) {
                    ...
                    $subQuery->whereNull('notification_key')
                        ->where('message', 'like', '%Peringatan: Masa PKS Anda akan berakhir%');
                });
            })
            ->delete();
    }
    ...
    {
        return "Peringatan: Masa PKS Anda akan berakhir pada {$expireDate->format('d-m-Y')}. Silakan lakukan perpanjangan atau proses berhenti.";
    }
}
