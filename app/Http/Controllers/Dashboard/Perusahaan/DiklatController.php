...
class DiklatController extends Controller
{
    ...
    // Untuk verifikasi diklat internal
    ...
    public function verifikasiTahap2(Request $request, $diklatId)
    {
        ...
        if ($request->has('verifikasi_kedua_disetujui') && $request->verifikasi_kedua_disetujui == 1) {
            ...
            if ($status_diklat_id == 2) {
                ...
                $users = User::whereIn('id', $totalPeserta)->get();
                foreach ($users as $user) {
                    $this->createNotifikasiDiklatInternal_Verif2($diklat, $user, 'Ditolak');
                }

                // Kirim juga ke user_id = 1 (Super Admin)
                $superAdmin = User::find(1);
                $this->createNotifikasiDiklatInternal_Verif2($diklat, $superAdmin, 'Ditolak', true);
    ...
    public function verifikasiTahap3(Request $request, $diklatId)
    {
        // 1. Dapatkan ID user yang login
        $verifikatorId = Auth::id();
        ...
        if (!Auth::user()->hasRole('Super Admin')) {
            ...
            if (!$relasiVerifikasi) {
                return response()->json([
                    'status' => Response::HTTP_NOT_FOUND,
                    'message' => "Anda tidak memiliki hak akses untuk verifikasi diklat internal tahap 3 dengan modul '{$relasiVerifikasi->modul_verifikasis->label}'.",
                    'relasi_verifikasi' => null,
                ], Response::HTTP_NOT_FOUND);
            }
            ...
            $userDiverifikasi = $relasiVerifikasi->user_diverifikasi;
            if (!is_array($userDiverifikasi)) {
                Log::warning('Kesalahan format data user diverifikasi pada verif 3 diklat internal');
                return response()->json(new WithoutDataResource(Response::HTTP_INTERNAL_SERVER_ERROR, 'Kesalahan format data user diverifikasi.'), Response::HTTP_INTERNAL_SERVER_ERROR);
            }
            ...
            if ($relasiVerifikasi->order != 3) {
                return response()->json(new WithoutDataResource(Response::HTTP_BAD_REQUEST, 'Diklat internal ini tidak dalam status untuk disetujui pada tahap 3.'), Response::HTTP_BAD_REQUEST);
            }
        }
        ...
        if ($request->has('verifikasi_ketiga_disetujui') && $request->verifikasi_ketiga_disetujui == 1) {
            if ($status_diklat_id == 4) {
                ...
                $users = User::whereIn('id', $totalPeserta)->get();
                foreach ($users as $user) {
                    $this->createNotifikasiDiklatInternal_Verif3($diklat, $user, 'Disetujui');
                }

                // Kirim juga ke user_id = 1 (Super Admin)
                $superAdmin = User::find(1);
                $this->createNotifikasiDiklatInternal_Verif3($diklat, $superAdmin, 'Disetujui', true);

                $message = "Verifikasi tahap 3 untuk Diklat '{$diklat->nama}' telah disetujui.";
            ...
            if ($status_diklat_id == 4) {
                ...
                $users = User::whereIn('id', $totalPeserta)->get();
                foreach ($users as $user) {
                    $this->createNotifikasiDiklatInternal_Verif3($diklat, $user, 'Ditolak');
                }

                // Kirim juga ke user_id = 1 (Super Admin)
                $superAdmin = User::find(1);
                $this->createNotifikasiDiklatInternal_Verif3($diklat, $superAdmin, 'Ditolak', true);

                return response()->json(new WithoutDataResource(Response::HTTP_OK, "Verifikasi tahap 3 untuk Diklat '{$diklat->nama}' telah ditolak."), Response::HTTP_OK);
            } else {
                return response()->json(new WithoutDataResource(Response::HTTP_BAD_REQUEST, "Diklat '{$diklat->nama}' tidak dalam status untuk ditolak pada tahap 3."), Response::HTTP_BAD_REQUEST);
            }
        } else {
            return response()->json(new WithoutDataResource(Response::HTTP_BAD_REQUEST, 'Aksi tidak valid.'), Response::HTTP_BAD_REQUEST);
        }
    ...
    // Notifikasi Diklat Internal Created by Admin 
    ...
    // Notifikasi Diklat External Created by Admin 
    ...
    // Notifikasi Diklat Internal Verif 1, 2, 3
    ...
    private function createNotifikasiDiklatInternal_Verif2($diklat, $user, $status, $isSuperAdmin = false)
    {
        try {
            ...
            Log::error('| Diklat Eksternal | - Error saat membuat notifikasi tahap 2: ' . $e->getMessage());
        }
    }

    private function createNotifikasiDiklatInternal_Verif3($diklat, $user, $status, $isSuperAdmin = false)
    {
        try {
            $statusText = $status === 'Disetujui' ? 'Disetujui' : 'Ditolak';
            $message = $isSuperAdmin
                ? "Notifikasi untuk Super Admin: Verifikasi tahap 3 untuk Diklat Internal '{$diklat->nama}' dari '{$user->nama}' telah {$statusText}."
                : "'{$user->nama}', Verifikasi tahap 3 untuk Diklat Internal '{$diklat->nama}' telah {$statusText}.";
            ...
            Log::error('| Diklat Eksternal | - Error saat membuat notifikasi tahap 3: ' . $e->getMessage());
        }
    }

    // Notifikasi Diklat Eksternal Verif 1, 2, 3
    ...
    private function createNotifikasiDiklatEksternal_Verif3($diklat, $user, $status, $isSuperAdmin = false)
    {
        try {
            $statusText = $status === 'Disetujui' ? 'Disetujui' : 'Ditolak';
            $message = $isSuperAdmin
                ? "Notifikasi untuk Super Admin: Verifikasi tahap 3 untuk Diklat Eksternal '{$diklat->nama}' dari '{$user->nama}' telah {$statusText}."
                : "'{$diklat->nama}', Verifikasi tahap 3 untuk Diklat Eksternal '{$diklat->nama}' telah {$statusText}.";
    ...
    // Untuk generate sertifikat diklat internal
    public function generateCertificate($diklatId)
    {
        ...
        if (!Auth::user()->hasRole('Super Admin')) {
            ...
            // Pastikan order verifikasi adalah 4 untuk pembuatan sertifikat
            if ($relasiVerifikasi->order != 4) {
